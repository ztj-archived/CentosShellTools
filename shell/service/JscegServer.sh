...
###############
# Name: JscegServer service management
...
Command=${1}
ServiceConfig="${2}"
ServiceBin="/data/bin/jcseg-server/jcseg-server.jar"
...
function start(){
    ...
    /bin/su - root -c "java -jar ${ServiceBin} ${ServiceConfig} &" &> ${ServiceLog}
}
function stop(){
